/**
 * Simplex noise 3D, inline de propósito: o flow field do background é o único
 * consumidor e o spec R.3 pede a landing sem dependência extra.
 *
 * Port enxuto do algoritmo de Ken Perlin na formulação de Stefan Gustavson
 * ("Simplex noise demystified"), domínio público. A tabela de permutação é
 * fixa — o campo é o mesmo a cada reload, o que é desejável: o movimento vem
 * da terceira dimensão (tempo), não de um seed aleatório.
 */

// Gradientes: as 12 arestas de um cubo.
...
// Duplicada para 512 posições: evita o `% 256` dentro do laço.
...
function dot(gi: number, x: number, y: number, z: number): number {
  const i = gi * 3
...
export function simplex3(xin: number, yin: number, zin: number): number {
  // Distorce o cubo de entrada para a grade simplex.
  ...
  // Em qual dos seis tetraedros caímos.
  ...
  if (t0 > 0) {
    t0 *= t0
    n += t0 * t0 * dot(PERM_MOD_12[ii + PERM[jj + PERM[kk]]], x0, y0, z0)
  }

  let t1 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1
  if (t1 > 0) {
    t1 *= t1
    n += t1 * t1 * dot(PERM_MOD_12[ii + i1 + PERM[jj + j1 + PERM[kk + k1]]], x1, y1, z1)
  }

  let t2 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2
  if (t2 > 0) {
    t2 *= t2
    n += t2 * t2 * dot(PERM_MOD_12[ii + i2 + PERM[jj + j2 + PERM[kk + k2]]], x2, y2, z2)
  }

  let t3 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3
  if (t3 > 0) {
    t3 *= t3
    n += t3 * t3 * dot(PERM_MOD_12[ii + 1 + PERM[jj + 1 + PERM[kk + 1]]], x3, y3, z3)
  }
